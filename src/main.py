...
def create_rejection_message(rejection_reasons):
    template = """Hello, your project has the following issues, please fix them for approval:
    ADD_REASONS_HERE 
    If you have any questions, DM @kaboom or create a ticket in #ask-the-shipwrights. """\

    # seperate rejection reasons with dashes
    bullet_rejection = "\n".join(f"- {item}" for item in rejection_reasons)

    template = template.replace("ADD_REASONS_HERE", bullet_rejection)
